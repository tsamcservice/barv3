      ...
      if (isTempMainCard && type === 'main') {
        ...
        try {
          ...
          if (configData) {
            ...
            console.log(`✅ simple-points讀取到動態初始點數: ${tempCurrentPoints}`);
          } else {
            console.log(`⚠️ simple-points未找到初始點數設定，使用fallback: ${tempCurrentPoints}`);
          }
        } catch (configError) {
          console.log(`⚠️ simple-points讀取初始點數設定失敗，使用fallback: ${tempCurrentPoints}`, configError);
        }
